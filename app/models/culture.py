...
class CultureBase(SQLModel):
    ...
    date_culture: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
class Culture(CultureBase, table=True):
    ...
    ferme: Ferme = Relationship(back_populates="cultures")
    utilisateur: Utilisateur = Relationship(back_populates="cultures")
