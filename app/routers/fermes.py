...
def create_ferme_endpoint(   
    ...
    if utilisateur_current.role == "Agriculteur" and utilisateur_current.id != ferme.utilisateur_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Vous ne pouvez créer une ferme que pour votre propre compte"
        )
    return create_ferme(session=session, ferme=ferme, utilisateur_current=utilisateur_current)
...
async def read_fermes_endpoint(
    ...
    if utilisateur_current.role == "Agriculteur":
        return read_ferme_by_utilisateur(session=session, utilisateur_id=utilisateur_current.id)
...
async def read_ferme_endpoint(
    ...
    if not ferme:
        raise HTTPException(
            ...
            detail="Ferme inexistante"
        )
    
    if ferme.utilisateur_id != utilisateur_current.id and utilisateur_current.role == "Agriculteur":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Vous n'avez pas les droits pour voir la ferme"
        )
    return ferme
...
async def read_fermes_by_utilisateur_endpoint(
    ...
    if utilisateur_current.id != utilisateur_id and utilisateur_current.role == "Agriculteur":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Vous n'avez pas les droits pour voir les fermes de cet utilisateur"
        )
...
async def update_ferme_endpoint(
    ...
    if utilisateur_current.id != ferme.utilisateur_id and utilisateur_current.role == "Agriculteur":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Vous n'avez pas les droits pour modifier la ferme"
        )
...
async def delete_ferme_endpoint(
    ...
    if utilisateur_current.id != ferme.utilisateur_id and utilisateur_current.role == "Agriculteur":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Vous n'avez pas les droits pour supprimer la ferme"
        )
