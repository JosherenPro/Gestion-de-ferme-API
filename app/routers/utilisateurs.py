...
async def read_utilisateurs_me(
    utilisateur_current: Annotated[Utilisateur, Depends(requires_role("Admin", "Agriculteur", "Technicien"))]
):
...
async def create_utilisateur_endpoint(
    session: Session = Depends(get_session),
    utilisateur: UtilisateurCreate = Body(...)
):
...
async def read_utilisateurs_endpoint(
    ...
    utilisateur_current: Utilisateur = Depends(requires_role("Admin", "Technicien"))
):
    if utilisateur_current.role not in ["Admin", "Technicien"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Vous n'avez pas les droits pour voir les utilisateurs"
        )
...
async def read_utilisateur_endpoint(
    ...
    utilisateur_current: Utilisateur = Depends(requires_role("Admin", "Technicien", "Agriculteur"))
):
    
    if utilisateur_current.id != utilisateur_id and utilisateur_current.role not in ["Admin", "Technicien"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Vous n'avez pas les droits pour modifier l'utilisateur"
        )
...
async def update_utilisateur_endpoint(
    ...
    utilisateur = update_utilisateur(session=session, utilisateur_id=utilisateur_id, utilisateur=utilisateur)
    if not utilisateur:
        raise HTTPException(status_code=404, detail="Utilisateur inexistant")
    return utilisateur
...
async def delete_utilisateur_endpoint(
    ...
    if utilisateur_current.id != utilisateur_id and utilisateur_current.role not in ["Admin", "Technicien"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Vous n'avez pas les droits pour supprimer l'utilisateur"
        )
