...
def read_cultures_by_ferme(
        *,
        session: Session,
        ferme_id: int
):
    cultures = session.exec(select(Culture).where(Culture.ferme_id == ferme_id)).all()
    return cultures

def read_cultures_by_utilisateur(
        *,
        session: Session,
        utilisateur_id: int
):
    cultures = session.exec(select(Culture).where(Culture.utilisateur_id == utilisateur_id)).all()
    return cultures
