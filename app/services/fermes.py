...
from app.schemas.ferme import FermeCreate, FermeUpdate


# creer une ferme
def create_ferme(*, session: Session, ferme: FermeCreate):
...
def read_ferme(*, session: Session, ferme_id: int):
...
def read_ferme_by_utilisateur(*, session: Session, utilisateur_id: int):
    script = select(Ferme).where(Ferme.utilisateur_id == utilisateur_id)
    fermes = session.exec(script).all()
    return fermes


# mettre a jour une ferme
def update_ferme(*, session: Session, ferme_id: int, ferme: FermeUpdate):
...
# supprimer une ferme
def delete_ferme(*, session: Session, ferme_id: int):
    ferme = session.get(Ferme, ferme_id)
    if not ferme:
        return None
